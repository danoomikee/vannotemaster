...
export const VideoPlayer = forwardRef<VideoPlayerHandle, VideoPlayerProps>(
  ({ src, youtubeId, annotations = [], onTimeUpdate, onAnnotationClick, onDurationChange, className = "" }, ref) => {
    ...
    return (
      <div ref={containerRef} className={`video-container max-h-full overflow-hidden ${className}`}>
        <div className="relative w-full h-full flex flex-col max-h-full">
          ...
          <div className="absolute bottom-16 left-0 right-0 h-2 pointer-events-none">
            {annotations.map((annotation) => {
              const leftPercent = duration > 0 ? (annotation.startTime / duration) * 100 : 0
              const widthPercent =
                annotation.endTime && duration > 0 ? ((annotation.endTime - annotation.startTime) / duration) * 100 : 0
