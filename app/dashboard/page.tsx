...
import { FileVideo, Plus, LayoutTemplate, Keyboard, FolderPlus } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const [showSeekToTimeModal, setShowSeekToTimeModal] = useState(false)
  ...
  const handleToggleCollectionSidebar = () => {
    ...
    if (selectedCollection) {
      ...
      const collectionToOpen = collections.find((c) => c.id === lastOpenedCollectionRef.current) || collections[0]
  ...
  useHotkeys(
    ...
    () => {
      // Seek to Previous
      const prevAnnotation = [...annotations].reverse().find((a) => a.startTime < currentTime)
      if (prevAnnotation && videoRef.current) videoRef.current.seek(prevAnnotation.startTime)
    },
    { enabled: !!currentVideo },
  )
  ...
  useHotkeys(
    ...
    () => {
      ...
      if (nextAnnotation && videoRef.current) videoRef.current.seek(nextAnnotation.startTime)
    },
    { enabled: !!currentVideo },
  )
  ...
  const handleVideoUpdate = async (videoId: string, data: Partial<Omit<Video, "id">>) => {
    ...
    try {
      if (editingCollection) {
        ...
        if (selectedCollection?.id === editingCollection.id) {
          const updatedCollection = await storage.getCollection(editingCollection.id)
  ...
  const handleCollectionInlineSave = async (data: Partial<Collection>) => {
    if (!currentVideo || !selectedCollection) return
    try {
      await storage.updateCollection(selectedCollection.id, data)
      const updatedCollection = await storage.getCollection(selectedCollection.id)
      if (updatedCollection) {
        setSelectedCollection(updatedCollection)
      }
      await loadCollections(currentVideo.id)
  ...
  const handleAddAnnotationToCollection = async (annotationId: string, collectionId: string) => {
    try {
  ...
  const handleRemoveAnnotationFromCollection = async (collectionId: string, annotationId: string) => {
    try {
      const collection = collections.find((c) => c.id === collectionId)
      if (!collection) return

      // Remove annotation from collection
      const updatedAnnotationIds = collection.annotationIds.filter((id) => id !== annotationId)
      await storage.updateCollection(collectionId, {
        annotationIds: updatedAnnotationIds,
      })
  ...
  const handleReorderAnnotations = async (collectionId: string, annotationIds: string[]) => {
    try {
  ...
  const handleTemplateSave = async (template: Omit<Template, "id" | "createdAt" | "updatedAt">) => {
    try {
      if (editingTemplate) {
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset className="flex flex-col h-screen overflow-hidden">
          ...
          {currentVideo ? (
              ...
              <div className="flex-1 min-h-0 flex overflow-hidden">
                <div className="flex-1 flex flex-col">
                  <VideoPlayer
                    ...
                    onDurationChange={setDuration} // Added onDurationChange callback
                    className="flex-1"
                  />
                  ...
                  <QuickAnnotationBar
                    ...
                    onDetailedAnnotate={() => handlePauseAndResume(handleDetailedAnnotate)}
                ...
                {selectedCollection && (
                  <div className="w-80 border-l bg-background">
                    <CollectionDetailView
                      ...
                      onSave={handleCollectionInlineSave}
                    />
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 flex items-center justify-center p-4 min-h-0">
              <div className="max-w-2xl w-full text-center">
                ...
                <h2 className="text-2xl font-semibold mb-2">Welcome to VanNote</h2>
                <p className="text-muted-foreground mb-6">
                  Create timestamp labels and segments on videos with customizable templates for structured annotation
                  data.
                </p>
        ...
        <SeekToTimeModal
          open={showSeekToTimeModal}
          onOpenChange={setShowSeekToTimeModal}
