import type { StorageAdapter, Video, Annotation, Template, Collection } from "../types"
...
export class LocalStorageAdapter implements StorageAdapter {
  ...
  async getAnnotationsByVideo(videoId: string): Promise<Annotation[]> {
    const annotations = this.getFromStorage<Annotation>("annotations")
    return annotations.filter((a) => a.videoId === videoId).sort((a, b) => a.startTime - b.startTime)
  }

  async getUnassignedAnnotations(videoId: string): Promise<Annotation[]> {
    const annotations = this.getFromStorage<Annotation>("annotations")
    return annotations.filter((a) => a.videoId === videoId && !a.collectionId).sort((a, b) => a.startTime - b.startTime)
  }
  ...
  async updateAnnotation(id: string, updates: Partial<Annotation>): Promise<Annotation> {
    ...
    annotations[index] = { ...annotations[index], ...updates, updatedAt: new Date() }
    this.saveToStorage("annotations", annotations)
  ...
  async deleteAnnotation(id: string): Promise<void> {
    ...
    if (annotation?.collectionId) {
      const collections = this.getFromStorage<Collection>("collections")
      const collection = collections.find((c) => c.id === annotation.collectionId)
      if (collection) {
        collection.annotationIds = collection.annotationIds.filter((aid) => aid !== id)
